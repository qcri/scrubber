...
def run():
    ...
    datasets_with_golden = set(['beer', 'bike', 'books', 'citations', 'movies', 'restaurants', 'fodors', 'cora'])
    ...
    datasets_with_manual = []

    for dataset_name in datasets_with_golden:
        config_file = os.path.join('/export/da/mkunjir/LabelDebugger/config', dataset_name + '.config')
        params = utils.myconfig.read_config(config_file)
        run_with_golden(params)

    for dataset_name in datasets_with_manual:
...
def debug_labels(debugger, index2pair, pair2golden):
    ...
    while True:
        ...
        iter_count, num_errors, error_indices, match_indices, det_error_poses  = debugger.analyze(index2correct_label)
