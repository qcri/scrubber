...
def read_tables():
    ...
    config_file=r'/export/da/mkunjir/LabelDebugger/config/songs.config'
    # read config
    ...
    print(dataset_name)
    # base dir
    basedir = params['basedir'] #r'/scratch/hzhang0418/projects/datasets/labeldebugger/songs_small'
    ...
    print("matches data: ", len(table_G))
...
def test():
    ...
    candidates = gen_candidates(table_A, table_G)
    gen_labeled_data(table_A, table_G, candidates)    
...
def gen_labeled_data(table_A, table_G, candidates):
    ...
    print("matches columns: ", len(id1_col), " and ", len(id2_col))
    
    matches = set()
    for v1,v2 in zip(id1_col, id2_col):
        if v1==v2:
            continue
        if (v2,v1) not in matches:
            matches.add((v1, v2))
    ...
    while True:
        ...
        p2 = (id_col[index_B], id_col[index_A]) # the reverse pair
        if p2 in candidates or p2 in pairs:
            continue
    ...
    #labeled_pairs_file = r'/scratch/hzhang0418/projects/datasets/labeldebugger/songs_small/labeled_data.csv'
    labeled_pairs_file=r'/export/da/mkunjir/LabelDebugger/datasets/songs/labeled_data.csv'
